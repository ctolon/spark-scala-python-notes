...
class MLPipelines(object):
    ...
    def rf_ml_pipeline(
        ...
        rf = RandomForestClassifier(featuresCol=rf_ss_output, labelCol=target_column, seed=1)
        ...
        # Pipelines
        machine_learning_pipeline = Pipeline(stages=[rf_si, rf_ohe, rf_va, rf_ss, rf])
        
        print(f"Machine Learning Pipeline : {machine_learning_pipeline.explainParams()}")
        print(f"Grid Params: {grid_params}")
        
        # Define the CrossValidator for each algorithm
        ...
        #rf_model = machine_learning_pipeline.fit(train_df)        
